...
int findNumsLessThanOrEqualTo(int digit, vector<int> &A, bool includeZero)
{
    int count = 0;
    for (int i = 0; i < A.size(); i++)
    {
        ...
        if (A[i] == 0)
        {
            count = includeZero ? count + 1 : count;
        }
        else
        {
            count++;
    ...
    cout << "Less than or equal values  =" << count << ", digit = " << digit << endl;
    return count;
}
bool numberPresent(vector<int> &A, int num)
{
    for (int i = 0; i < A.size(); i++)
    {
        if (A[i] == num)
...
int solve(vector<int> &A, int B, int C)
{
    if (B == 0 || A.size() == 0)
        return 0;
    int n = A.size();
    vector<int> digitsInC;
    while (C)
    {
        digitsInC.push_back(C % 10);
        C /= 10;
    }
    int deduct = 1;
    int numCombos = 0;
    if (digitsInC.size() < B)
        return 0;
    else if (digitsInC.size() > B)
    {
        int firstPower = (B == 1) ? n : (A[0] ? n : n - 1);
        numCombos = firstPower * pow(n, B - 1);
        B = 0;
    }
    else
    {
        bool includeZero = (digitsInC.size() == 1);
        while (B && (digitsInC.size() > 0))
        {
            int curDigit = digitsInC.back();
            digitsInC.pop_back();
            int numLTE = findNumsLessThanOrEqualTo(curDigit - 1, A, includeZero);
            cout << "numLTE = " << numLTE << ",n=" << n << ",power = " << digitsInC.size() << endl;
            int temp = numLTE * pow(n, digitsInC.size());
            cout << "numLte*pow(n,digits)=" << temp << endl;
            numCombos += temp;
            if (!numberPresent(A, curDigit))
            {
                B = 0;
                break;
            }
